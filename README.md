...
## 更新日志
## test2.6 2026-07-07
- 修复 Web API 回调中的任务看门狗误复位：未注册到 TWDT 的任务不再调用 `esp_task_wdt_reset()`，消除 `task not found` 错误日志；
- 修复无信号覆盖逻辑导致的“设置模式后 RGB 不亮”问题：仅在实际覆盖到电量通道时才跳过常规渲染，避免非电量模式被卡黑；
...
8. **自动循环** (7): 自动切换各种效果
9. **电量显示** (8): 显示电池电量
10. **音乐律动1** (9): 随音乐节奏律动（背景叠加）
11. **音乐律动2** (10): 音量条模式（可切换彩色/单色）
