...
"    var html = '<b>版本:</b> ' + d.version + '<br>' +"
"      '<b>自动更新:</b> ' + (d.auto_update ? '✅ 已开启' : '❌ 已关闭') + '<br>' +"
"      '<b>最近状态:</b> ' + d.last_status;"
"    if (d.pending_update && d.pending_update.waiting_confirmation) {"
"      html += '<br><br><span style=\"color:red;font-weight:bold;\">🎯 发现新版本: ' + d.pending_update.version + '</span>';"
"      html += '<br>下载地址: ' + d.pending_update.url;"
"      html += '<br><button style=\"margin-top:10px;background:green;\" onclick=\"confirmOtaUpdate()\">确认更新</button>';"
"    }"
"    html += '<br><br><span style=\"font-size:0.9em;color:#666;\">空闲堆内存: ' + (d.free_heap_bytes/1024).toFixed(1) + ' KB</span>';"
"    document.getElementById('ota-info').innerHTML = html;"
...
"  document.getElementById('ota-info').innerHTML = '🔄 正在检查更新...';"
...
"        fetch('/api/ota/status').then(function(r){return r.json();}).then(function(statusData){"
"          console.log('OTA检查状态:', statusData.last_status);"
"          if (statusData.check_in_progress && retryCount < maxRetries) {"
"            // 检查仍在进行中，继续等待"
"            retryCount++;"
...
"    } else {"
"      alert('检查失败: ' + (d.message || '未知错误'));"
"      console.error('OTA check error:', d);"
"      refreshOtaStatus();"
"    }"
"  }).catch(function(e){"
"    alert('网络错误: ' + e.message);"
"    console.error('OTA check request failed:', e);"
"    refreshOtaStatus();"
"  });"
"}"
"function confirmOtaUpdate() {"
"  if (!confirm('确认开始下载并安装更新吗?')) return;"
"  document.getElementById('ota-info').innerHTML = '⏳ 正在下载更新...';"
"  fetch('/api/ota/confirm', {method:'POST',headers:{'Content-Type':'application/json'},body:'{}'})"
"    .then(function(r){return r.json();})"
"    .then(function(d){"
"      if (d.ok) {"
"        document.getElementById('ota-info').innerHTML = '✅ 更新已开始，设备将在完成后重启。新版本: ' + d.version;"
"        console.log('OTA update confirmed and started for version', d.version);"
"        setTimeout(refreshOtaStatus, 5000);"
"      } else {"
"        alert('无法启动更新: ' + (d.message || '未知错误'));"
"        refreshOtaStatus();"
"      }"
"    })"
"    .catch(function(e){"
"      alert('确认失败: ' + e.message);"
"      console.error('OTA confirm failed:', e);"
"      refreshOtaStatus();"
"    });"
"}"
